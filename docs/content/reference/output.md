...
describes what users get. Pick a format with `-o`, or let 24h choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
24h <command> -o table   # aligned columns for reading
24h <command> -o jsonl   # one JSON object per line, for piping
24h <command> -o json    # a single JSON array
24h <command> -o csv     # spreadsheet friendly
24h <command> -o tsv     # tab-separated
24h <command> -o url     # just the URL column
24h <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
24h <command> --fields id,title,url
```
...
```bash
24h <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
24h <command>            # a table, because this is a terminal
24h <command> | wc -l    # JSONL, because this is a pipe
```
